...
#define hdbWarning(fmt, ...) hdb::__warning(__FILE__, __LINE__, fmt, ##__VA_ARGS__, 0)
...
static inline int __error(const char* file, unsigned int line, const char* fmt, Args... args) {
	fprintf(stderr, "\e[31mError: \033[0mIn file: %s:%d\n", file, line);
	return fprintf(stderr, fmt, args...);
}

template <typename... Args>
static inline int error(const char* fmt, Args... args) {
	fprintf(stderr, "\e[31mError: \033[0m");
	return fprintf(stderr, fmt, args...);
}

template <typename... Args>
static inline int __warning(const char* file, unsigned int line, const char* fmt, Args... args) {
	fprintf(stderr, "\e[33mWarning: \033[0mIn file: %s:%d\n", file, line);
	return fprintf(stderr, fmt, args...);
}

template <typename... Args>
static inline int warning(const char* fmt, Args... args) {
	fprintf(stderr, "\e[33mWarning: \033[0m");
	return fprintf(stderr, fmt, args...);
}
